...
def signup(body: SignupInput, request: Request, db: Session = Depends(db_session)):
    rate_limiter.check(f"signup:{request.client.host if request.client else 'unknown'}", 5, 3600)
    ...
    db.add(user)
    db.flush()
    audit(db, request, "user_signed_up", target=user, after=snapshot(user))
...
def login(body: LoginInput, request: Request, response: Response, db: Session = Depends(db_session)):
    rate_limiter.check(f"login:{request.client.host if request.client else 'unknown'}", 10, 900)
    ...
    if not user or not valid:
        audit(db, request, "login_failed", target=user)
        db.commit()
    ...
    if user.status != "active":
        ...
        audit(db, request, "login_blocked_status", target=user, after={"status": user.status})
        db.commit()
    ...
    audit(db, request, "login_succeeded", actor=user, target=user)
    db.commit()
    response.set_cookie(
        "veritas_session",
        raw,
...
def logout(
    ...
    if veritas_session:
        ...
        if session:
            ...
            audit(db, request, "logout", target=db.get(User, session.user_id))
            db.commit()
...
def _update_profile(body: ProfileUpdate, request: Request, user: User, db: Session) -> dict:
    rate_limiter.check(f"profile:{request.client.host if request.client else 'unknown'}", 20, 900)
    ...
    audit(db, request, "profile_updated", actor=user, target=user, before=before, after=snapshot(user))
    db.commit()
    db.refresh(user)
...
def verify_email(body: TokenInput, request: Request, db: Session = Depends(db_session)):
    rate_limiter.check(f"verify:{request.client.host if request.client else 'unknown'}", 10, 900)
    record = db.scalar(
    ...
    audit(db, request, "email_verified", target=user, before=before, after=snapshot(user))
...
def resend_verification(body: EmailInput, request: Request, db: Session = Depends(db_session)):
    rate_limiter.check(f"resend:{request.client.host if request.client else 'unknown'}", 3, 3600)
    user = db.scalar(select(User).where(User.email == normalize_email(str(body.email))))
    if user and user.status == "pending_verification":
        send_token_email(db, user, "verification")
        audit(db, request, "verification_resent", target=user)
        db.commit()
...
def password_reset_request(body: EmailInput, request: Request, db: Session = Depends(db_session)):
    rate_limiter.check(f"reset:{request.client.host if request.client else 'unknown'}", 3, 3600)
    user = db.scalar(select(User).where(User.email == normalize_email(str(body.email))))
    if user and user.status == "active":
        send_token_email(db, user, "password_reset")
        audit(db, request, "password_reset_requested", target=user)
        db.commit()
...
def password_reset_confirm(body: ResetInput, request: Request, db: Session = Depends(db_session)):
    rate_limiter.check(f"reset-confirm:{request.client.host if request.client else 'unknown'}", 5, 900)
    record = db.scalar(
        select(OneTimeToken).where(
    ...
    audit(db, request, "password_reset_completed", target=user)
